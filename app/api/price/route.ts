...
async function binancePrice(symbol: string): Promise<number> {
  ...
  const n = Number(j.price); if (!Number.isFinite(n)) throw new Error('NaN'); return n;
}
async function symbolExists(symbol: string): Promise<boolean> {
  ...
  const ok = res.ok; map.set(symbol, ok); return ok;
}
export async function GET(req: Request) {
  ...
  try { const [pa, pb] = await Promise.all([ needA ? binancePrice(sa) : Promise.resolve(hitA!.price), needB ? binancePrice(sb) : Promise.resolve(hitB!.price) ]);
